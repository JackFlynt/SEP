...
namespace Shared.CameraTools
{
    public class Camera
    {
        ...
        public void ShowExtent(Vector2f min, Vector2f max)
        {
            var difference = max - min;
            var xDifference = max.X - min.X;
            var yDifference = max.Y - min.Y;

            var currentAspectRatio = view.Size.X / view.Size.Y;

            float newXSize = -1;
            float newYSize = -1;

            if (xDifference / yDifference < currentAspectRatio)
            {
                // the new y is bigger than the new x
                newXSize = yDifference * currentAspectRatio;
                newYSize = yDifference;
            }
            else
            {
                newXSize = xDifference;
                newYSize = xDifference / currentAspectRatio;
            }

            this.view.Size = new Vector2f(newXSize, newYSize);
            this.SetCentre(min + difference / 2);
        }
